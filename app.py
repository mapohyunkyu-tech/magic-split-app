...
APP_VERSION = "v4_TOP50_완화_진단_REAL"
...
def build_universe(asof_date, price_limit, max_codes):
    ...
    for market in ["KOSPI", "KOSDAQ"]:
        try:
            ...
            # pykrx 버전별 index 컬럼명이 달라질 수 있어 첫 컬럼을 Code로 통일
    ...
    if len(rows) == 0:
        universe = {str(code).zfill(6): name for code, name in FORCE_UNIVERSE.items()}
...
st.caption(APP_VERSION)
    ...
    st.info("후보가 안 뜨면 계산 종목수 700으로 두고 실행하세요. v4는 엄격 후보가 0개면 완화 후보를 자동으로 보여줍니다.")

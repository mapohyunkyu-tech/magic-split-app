...
def find_valid_krx_date(end_date=None, max_back=20):
    if end_date is None:
    ...
    for _ in range(max_back):
        ds = d.strftime("%Y%m%d")
        try:
            tickers = stock.get_market_ticker_list(ds, market="ALL")
            if len(tickers) > 100:
                return ds
        except Exception:
            pass
...
def build_universe(asof_date, price_limit, max_codes):
    ...
    for market in ["KOSPI", "KOSDAQ"]:
        try:
            ...
            if snap is None or len(snap) == 0:
                continue
            snap = snap.reset_index().rename(columns={"티커": "Code", "종가": "Close", "거래대금": "Amount"})
            if "Code" not in snap.columns:
                first_col = snap.columns[0]
                snap = snap.rename(columns={first_col: "Code"})
    ...
    df = df[df["Close"] <= price_limit]
    df = df[df["Amount"] >= 300_000_000]
...
# =====================================================
    ...
    with col8:
        max_codes = st.number_input("계산 종목수", min_value=50, max_value=700, value=250, step=50)

    st.info("무료 서버에서는 계산 종목수를 150~250으로 먼저 테스트하세요. 너무 크게 잡으면 느릴 수 있습니다.")
    ...
    if st.button("TOP50 생성", type="primary"):
        ...
        universe = build_universe(asof_date, price_limit, int(max_codes))
